...
class Stratum {
  ...
  public function __construct() {
    ...
    socket_bind($this->p[0], 0, 8033) or die('ERROR: Could not bind to address.');
  ...
  private function x($r) {
    if (in_array($this->p[0], $r)) {
      ...
      if ($a=='127.0.0.1')
        socket_write($k, $this->h(@socket_read($k, 2048, PHP_NORMAL_READ))."\n");
      ...
      if ($_d === FALSE) $this->k($k, 'lost');
        ...
        if (($d = json_decode($_d)) && isset($d->method)) {
          if ($d->method == 'mining.subscribe') {
            ...
            $known = (isset($d->params) && isset($d->params[0]) && $d->params[0]);
            if ($known) {
              $this->o[$k]->u = $d->params[0];
              $this->o[$k] = new U($this->o[$k]);
              if (!$this->p[$k]) {
  ...
  private function h($h) {
    $this->l('HTTP says '.$h);
    $d = array('result'=>FALSE);
    if (is_object($h = @json_decode($h)) && isset($h->method))
      switch($h->method) {
        case 'wtf':
          if (isset($h->params) && isset($h->params[0]))
            $d['result'] = $h->params[0].' + '.$h->params[0].'='.($h->params[0] + $h->params[0]);
          break;
      }
...
class U {
  ...
  public function c($p) {
    if (isset($this->p[$this->u]) && isset($this->p[$this->u]['p']))
      foreach($this->p[$this->u]['p'] as $_p)
        if (socket_connect($p, $_p[0], $_p[1])) return $this->P = $_p;
    if (socket_connect($p, $this->P[0], $this->P[1])) return $this->P;
    return FALSE;
  }
  public function d($d) {
    return $this->P ? strtr($d, array($this->u => $this->P[2])) : $d;
  }
}
